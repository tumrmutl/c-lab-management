...
function handleCSVUpload($csvFile, $conn) {
    ...
    while (($row = fgetcsv($csvFile)) !== FALSE) {
        // Logging each row to ensure it's being read properly
        error_log("Processing row: " . implode(', ', $row));

        $std_id = explode('_', $row[0])[0]; // Extract only the part before '_'
        $lab_id = $row[1];
        $student_output = $conn->real_escape_string($row[2]); // Escaping strings for safety
        $teacher_output = $conn->real_escape_string($row[3]);
        $result = $row[4];
